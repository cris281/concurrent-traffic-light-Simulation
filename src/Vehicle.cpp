        ...
        if (timeSinceLastUpdate >= cycleDuration)
        {
            ...
            if (completion >= 0.9 && !hasEnteredIntersection)
            {
                ...
                try
                {
                    ...
                    std::cout << "[LOG] Vehicle #" << _id << " attempting to enter Intersection #" << _currDestination->getID() << ", use_count=" << self.use_count() << std::endl;
                    auto ftrEntryGranted = std::async(&Intersection::addVehicleToQueue, _currDestination, self);
                ...
                {
                    std::cerr << "[ERROR] bad_weak_ptr in Vehicle::drive (entering intersection) for Vehicle #" << _id << ": " << e.what() << std::endl;
                    throw;
                }
            ...
            if (completion >= 1.0 && hasEnteredIntersection)
            {
                ...
                {
                    auto self = get_shared_this();
                    std::cout << "[LOG] Vehicle #" << _id << " leaving Intersection #" << _currDestination->getID() << ", use_count=" << self.use_count() << std::endl;
                    _currDestination->vehicleHasLeft(self);
                }
                catch (const std::bad_weak_ptr &e)
                {
                    std::cerr << "[ERROR] bad_weak_ptr in Vehicle::drive (leaving intersection) for Vehicle #" << _id << ": " << e.what() << std::endl;
                    throw;
                }
